...
// every child route. Live updates replace the cache before invalidating loads.
export const ssr = false;

export const load: LayoutLoad = async ({ data, depends }) => {
  depends('app:public-data');
  const snapshot = await getPublicSnapshot({ check: false });
  const activeSeasonId =
    snapshot.seasons.find((season) => season.is_active === true && season.is_casual !== true)?.id ??
    null;

  return { ...data, activeSeasonId, publicDataRevision: String(snapshot.revision.revision) };
};
